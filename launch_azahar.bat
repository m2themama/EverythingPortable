...
:Menu
cls
title Portable Azahar Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall azahar [will remove azahar entirely]
echo 2. launch azahar [launches azahar]
echo 3. reset azahar [will remove everything azahar except the binary]
echo 4. uninstall azahar [dislike the inability to use .3ds files?]
...
echo d. check for new azahar version [automatically check for a new version]
...
:ReinstallAzahar
cls
call :UninstallAzahar
call :UpgradeAzahar
exit /b 2

:2
:LaunchAzahar
if not exist ".\bin\azahar\azahar.exe" set "nag=PLEASE INSTALL AZAHAR FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo AZAHAR IS RUNNING
set "Path=!path!;!folder!\dll\64\;"
start .\bin\azahar\azahar.exe
exit
...
:ResetAzahar
cls
taskkill /f /im azahar.exe
if exist .\data\azahar\ rmdir /s /q .\data\azahar\
exit /b 2
...
cls
taskkill /f /im azahar.exe
if exist .\bin\azahar\ rmdir /s /q .\bin\azahar\
if exist .\extra\azahar-x64-5.0.exe del .\extra\azahar-x64-5.0.exe >nul
exit /b 2
...
cls
title Portable Azahar Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Azahar Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Azahar Launcher - Helper Edition - Download Suite
...
title Portable Azahar Launcher - Helper Edition - Quicklauncher Writer
...
echo start .\bin\azahar\azahar.exe>>!quick_launcher!
...
:UpgradeAzahar
cls
title Portable Azahar Launcher - Helper Edition - Azahar Update Check
if exist index.html del index.html >nul
call :HelperDownload "https://github.com/azahar-emu/azahar/releases/" "index.html"
for /f tokens^=2delims^=^" %%A in (
  'findstr /i /c:"-windows-msvc.zip" index.html'
) Do > .\doc\azahar_link.txt Echo:%%A & goto :stop_search
:stop_search
set /p azahar_link=<.\doc\azahar_link.txt
set "azahar_link=https://github.com!azahar_link:~0,-1!"
set "tempstr=!azahar_link!"
set "result=%tempstr:/=" & set "result=%"
set "azahar_zip=!result!"
set "azahar_dir=!azahar_zip:~0,-4!"
if exist index.html del index.html >nul
if "!Debug!" EQU "1" (
  cls
  echo "!azahar_link!"
...
if exist "!azahar_zip!" del "!azahar_zip!" >nul
if exist ".\extra\!azahar_zip!" (
  echo azahar is updated.
  pause
  exit /b
)
call :HelperDownload "!azahar_link!" "!azahar_zip!"
:MoveAzahar
move "!azahar_zip!" ".\extra\!azahar_zip!"
:ExtractAzahar
call :HelperExtract "!CD!\extra\!azahar_zip!" "!CD!\temp\"
xcopy ".\temp\!azahar_dir!\*" .\bin\azahar\ /e /i /y
if exist .\temp\ rmdir /s /q .\temp\
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!azahar_zip!")
exit /b 2

:e
title Portable Azahar Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\azahar\azahar.exe" set nag=AZAHAR IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Azahar Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Azahar Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Azahar Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Azahar Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Azahar Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Azahar Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
