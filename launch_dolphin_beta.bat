...
:Menu
cls
title Portable Dolphin Beta Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall dolphin [will remove dolphin entirely]
echo 2. launch dolphin [launches dolphin]
echo 3. reset dolphin [will remove everything dolphin except the binary]
echo 4. uninstall dolphin [real wii's are cheaper to get than a gaming rig]
...
echo d. check for new dolphin version [automatically check for a new version]
...
:1
:ReinstallDolphinBeta
cls
call :UninstallDolphinBeta
call :UpgradeDolphinBeta
exit /b 2

:2
:LaunchDolphinBeta
if not exist ".\bin\dolphin_beta\dolphin.exe" set "nag=PLEASE INSTALL DOLPHIN BETA FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo DOLPHIN BETA IS RUNNING
set "Path=!path!;!folder!\dll\64\;"
start .\bin\dolphin_beta\dolphin.exe -u "!folder!\data\dolphin"
exit
...
:UninstallDolphinBeta
cls
taskkill /f /im dolphin.exe
if exist .\bin\dolphin_beta\ rmdir /s /q .\bin\dolphin_beta\
if exist .\extra\dolphin-x64-5.0.exe del .\extra\dolphin-x64-5.0.exe >nul
exit /b 2
...
cls
title Portable Dolphin Beta Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Dolphin Beta Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Dolphin Beta Launcher - Helper Edition - Download Suite
...
title Portable Dolphin Beta Launcher - Helper Edition - Quicklauncher Writer
...
echo start .\bin\dolphin_beta\dolphin.exe -u "%%folder%%\data\dolphin">>!quick_launcher!
...
:e
title Portable Dolphin Beta Launcher - Helper Edition - Text-Reader Update Check
cls
...
:f
cls
for %%A in (!folder!\data\dolphin\Config\Dolphin.ini) do (
  set "A=%%A"
  for /f "DELIMS=" %%B in (%%A) do (
    set "B=%%B"
    if "!B:~0,7!" EQU "ISOPath" (
      set "C=!B:~11!"
      if "!C:~0,1!" NEQ "C" (
        if "!C:~0,1!" NEQ "A" (
          if "!C:~0,1!" NEQ "B" (
...
if not exist ".\bin\dolphin_beta\dolphin.exe" set nag=DOLPHIN BETA IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Dolphin Beta Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Dolphin Beta Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Dolphin Beta Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Dolphin Beta Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Dolphin Beta Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Dolphin Beta Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
