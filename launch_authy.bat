...
:2
:LaunchAuthy
if not exist ".\bin\authy\Authy Desktop.exe" set "nag=PLEASE INSTALL AUTHY FIRST" & exit /b 2
title DO NOT CLOSE
...
cls
taskkill /f /im "Authy Desktop.exe"
if exist .\bin\authy\ rmdir /s /q .\bin\authy\
exit /b 2
...
echo start "" ".\bin\authy\Authy Desktop.exe">>!quick_launcher!
...
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\Authy Desktop Setup !cycle1!.!cycle2!.!cycle3!.exe" ".\temp\")
exit /b 2
...
set "NullExtra=" & for /F "skip=1 delims=" %%j in (.\ini\settings.ini) do ( set "NullExtra=%%j" & set "NullExtra=!NullExtra:~-1!" & goto :Setting2 )
:Setting2
if "!Count!" LSS "4" (
