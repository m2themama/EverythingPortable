...
:Menu
cls
title Portable LibreOffice Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall libreoffice [will remove libreoffice entirely]
echo 2. launch libreoffice [launches libreoffice]
echo 3. reset libreoffice [will remove everything libreoffice except the binary]
echo 4. uninstall libreoffice [not enough features?]
...
echo d. check for new libreoffice version [automatically check for a new version]
echo e. install text-reader [update if had]
echo.
echo f. launch sbase [launches base database]
echo g. launch scalc [launches calc spreadsheet]
echo h. launch sdraw [launches draw drawing]
echo i. launch simpress [launches impress presentation]
echo j. launch smath [launches math formula]
echo k. launch swriter [launches writer document]
echo.
...
:1
:ReinstallLibreOffice
cls
call :UninstallLibreOffice
call :UpgradeLibreOffice
exit /b 2

:2
:LaunchLibreOffice
if not exist ".\bin\libreoffice\program\soffice.exe" set "nag=PLEASE INSTALL LIBREOFFICE FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo LIBREOFFICE IS RUNNING
start .\bin\libreoffice\program\soffice.exe
exit
...
:ResetLibreOffice
cls
taskkill /f /im LibreOffice.exe
if exist .\data\Users\MarioMasta64\AppData\Roaming\LibreOffice\ del .\data\Users\MarioMasta64\AppData\Roaming\LibreOffice\ >nul
exit /b 2
...
cls
taskkill /f /im LibreOffice.exe
for /d %%i in (".\bin\libreoffice\*") do rmdir /s /q "%%i"
if exist .\extra\*LibreOffice*.msi del .\extra\*LibreOffice*.msi >nul
pause
exit /b 2
...
cls
title Portable LibreOffice Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable LibreOffice Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable LibreOffice Launcher - Helper Edition - Download Suite
...
title Portable LibreOffice Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo start .\bin\libreoffice\program\soffice.exe>>!quick_launcher!
...
cls
:UpgradeLibreOffice
title Portable LibreOffice Launcher - Helper Edition - LibreOffice Update Check
if exist index.html del index.html >nul
call :HelperDownload "https://www.libreoffice.org/download/download-libreoffice/" "index.html"
set counter=0
:UpgradeSearchLoop
set /a counter+=1
for /f tokens^=%counter%delims^=^" %%A in (
  'findstr /i /c:"/download/download-libreoffice/?type=win-x86_64" index.html'
) Do > .\doc\libreoffice_link.txt Echo:%%A
set /p libreoffice_link=<.\doc\libreoffice_link.txt
if "!Debug!" EQU "1" (
  echo !libreoffice_link!
  echo !counter!
)
REM ></i>Windows Aarch64</a></li><li><a href='/download/download-libreoffice/?type=win-x86_64&version=26.2.0&lang=en-US'><i class=

if "!libreoffice_link:~42,-37!"=="/download/download-libreoffice/?type=win-x86_64" ( if "!Debug!" EQU "1" ( echo hit ) ) & goto ExitUpgradeSearchLoop
goto UpgradeSearchLoop
:ExitUpgradeSearchLoop
if exist index.html del index.html >nul
set "libreoffice_link=!libreoffice_link:~42,-11!"
set "tempstr=!libreoffice_link!"
set "result=%tempstr:/=" & set "result=%"
set "libreoffice_download=!result!"
set "libreoffice_link=https://download.documentfoundation.org/libreoffice/stable/!libreoffice_download:~25,-11!/win/x86_64/LibreOffice_!libreoffice_download:~25,-11!_Win_x86-64.msi"
set "libreoffice_msi=LibreOffice_!libreoffice_download:~25,-11!_Win_x86-64.msi"
if "!Debug!" EQU "1" (
  cls
  echo "!libreoffice_download:~25,-11!"
  echo "!libreoffice_download!"
  echo "!libreoffice_msi!"
  echo "!libreoffice_link!"
  echo PRESS ENTER TO CONTINUE & pause >nul
)
if exist index.html del index.html >nul
cls
if exist ".\extra\!libreoffice_msi!" (
  echo libreoffice is updated.
  pause
  exit /b 2
)
cls
echo upgrading to libreoffice v!libreoffice_download:~25,-11!
call :HelperDownload "!libreoffice_link!" "!libreoffice_msi!"
:MoveLibreOffice
move "!libreoffice_msi!" ".\extra\!libreoffice_msi!"
:ExtractLibreOffice
call :HelperExtractMSI "!folder!\extra\!libreoffice_msi!" "!folder!\temp\"
xcopy "!folder!\temp\SourceDir\LibreOffice\*" ".\bin\libreoffice\" /E /I /Y
if exist .\temp\ rmdir /s /q .\temp\
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!libreoffice_msi!")
exit /b 2

:e
title Portable LibreOffice Launcher - Helper Edition - Text-Reader Update Check
cls
...
:f
start .\bin\libreoffice\program\sbase.exe
exit /b 2

:g
start .\bin\libreoffice\program\scalc.exe
exit /b 2

:h
start .\bin\libreoffice\program\sdraw.exe
exit /b 2

:i
start .\bin\libreoffice\program\simpress.exe
exit /b 2

:j
start .\bin\libreoffice\program\smath.exe
exit /b 2

:k
start .\bin\libreoffice\program\swriter.exe
exit /b 2
...
if not exist ".\bin\libreoffice\program\soffice.exe" set nag=LIBREOFFICE IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable LibreOffice Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable LibreOffice Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable LibreOffice Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable LibreOffice Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable LibreOffice Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable LibreOffice Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
