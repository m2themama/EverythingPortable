...
:Menu
cls
title Portable Eden Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall eden [will remove eden entirely]
echo 2. launch eden [launches eden]
echo 3. reset eden [will remove everything eden except the binary]
echo 4. uninstall eden [tired of fighting nintendo lawyers?]
...
echo d. check for new eden version [automatically check for a new version]
...
:2
:LaunchEden
if not exist ".\bin\eden\eden.exe" set "nag=PLEASE INSTALL EDEN FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo EDEN IS RUNNING
set "Path=!path!;!folder!\dll\64\;"
start .\bin\eden\eden.exe
exit
...
if exist .\data\eden\ rmdir /s /q .\data\eden\
exit /b 2
...
cls
taskkill /f /im eden.exe
if exist .\bin\eden\ rmdir /s /q .\bin\eden\
if exist .\extra\eden-x64-5.0.exe del .\extra\eden-x64-5.0.exe >nul
exit /b 2
...
cls
title Portable Eden Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Eden Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Eden Launcher - Helper Edition - Download Suite
...
title Portable Eden Launcher - Helper Edition - Quicklauncher Writer
...
echo start .\bin\eden\eden.exe>>!quick_launcher!
...
:stop_search
set /p eden_link=<.\doc\eden_link.txt
set "eden_link=!eden_link:~0,-1!"
set "tempstr=!eden_link!"
set "result=%tempstr:/=" & set "result=%"
set "eden_zip=!result!"
set "eden_dir=!eden_zip:~0,-4!"
if exist index.html del index.html >nul
if "!Debug!" EQU "1" (
  cls
  echo "!eden_link!"
...
if exist "!eden_zip!" del "!eden_zip!" >nul
if exist ".\extra\!eden_zip!" (
  echo eden is updated.
  pause
  exit /b
)
call :HelperDownload "!eden_link!" "!eden_zip!"
:MoveEden
...
:e
title Portable Eden Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\eden\eden.exe" set nag=EDEN IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Eden Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Eden Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Eden Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Eden Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Eden Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Eden Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
