...
:Menu
cls
title Portable Xemu Launcher - Helper Edition - Main Menu
echo !NAG!
...
:2
:LaunchXemu
if not exist ".\bin\xemu\xemu.exe" set "nag=PLEASE INSTALL XEMU FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo XEMU IS RUNNING
...
:UninstallXemu
cls
set /p xemu_version=<.\doc\xemu_version.txt
taskkill /f /im "xemu.exe"
if exist .\bin\xemu\ rmdir /s /q .\bin\xemu\
if exist .\extra\xemu* del .\extra\xemu* >nul
exit /b 2
...
cls
title Portable Xemu Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Xemu Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Xemu Launcher - Helper Edition - Download Suite
...
title Portable Xemu Launcher - Helper Edition - Quicklauncher Writer
...
echo cls>>!quick_launcher!
echo cd .\bin\xemu\>>!quick_launcher!
echo start "" "xemu.exe">>!quick_launcher!
...
cls
:UpgradeXemu
title Portable Xemu Launcher - Helper Edition - Xemu Update Check
if exist latest del latest >nul
call :HelperDownload "https://api.github.com/repos/xemu-project/xemu/releases/latest" "latest"
:: create file or wont work (do not run on same file)
:: create file or wont work (do not run on same file)
echo.> latest.txt
TYPE latest | MORE /P > latest.txt
for /f tokens^=4delims^=^" %%A in (
  'findstr /i /c:"-windows-x86_64.zip" latest.txt'
) Do > .\doc\xemu_link.txt Echo:%%A
if exist latest del latest >nul
if exist latest.txt del latest.txt >nul
set /p xemu_link=<.\doc\xemu_link.txt
set "xemu_link=!xemu_link:.sha256=!"
set "tempstr=!xemu_link!"
...
if exist "!xemu_exe!" "!xemu_exe!" >nul
if exist ".\extra\!xemu_exe!" (
  echo xemu is updated.
  pause
  exit /b 2
)
cls
echo upgrading to xemu v!xemu_exe:~5,-19!
call :HelperDownload "!xemu_link!" "!xemu_exe!"
:MoveXemu
move "!xemu_exe!" ".\extra\!xemu_exe!"
:ExtractXemu
call :HelperExtract7zip "!folder!\extra\!xemu_exe!" "!folder!\bin\xemu\"
:NullExtra
if "!NullExtra!" EQU "1" ( echo.>".\extra\!xemu_exe!")
exit /b 2
...
if not exist ".\bin\xemu\xemu.exe" set nag=XEMU IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Xemu Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Xemu Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Xemu Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Xemu Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Xemu Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Xemu Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
