...
:Menu
cls
title Portable Gimp Launcher - Helper Edition - Main Menu
echo !NAG!
set nag="Selection Time!"
echo 1. reinstall gimp [will remove gimp entirely]
echo 2. launch gimp [launches gimp]
echo 3. reset gimp [will remove everything gimp except the binary]
echo 4. uninstall gimp [doesn't do everything you need it to do?]
...
echo d. check for new gimp version [automatically check for a new version]
...
:2
:LaunchGimp
if not exist ".\bin\gimp\bin\gimp*.exe" set "nag=PLEASE INSTALL GIMP FIRST" & exit /b 2
title DO NOT CLOSE
cls
echo GIMP IS RUNNING
set /p gimp_version=<.\doc\gimp_version.txt
start "" ".\bin\gimp\bin\gimp-!gimp_version!.exe"
exit
...
cls
set /p gimp_version=<.\doc\gimp_version.txt
taskkill /f /im "gimp-!gimp_version!.exe"
if exist .\data\Users\MarioMasta64\AppData\Local\GIMP\ rmdir /s /q .\data\Users\MarioMasta64\AppData\Local\GIMP\
if exist .\data\Users\MarioMasta64\AppData\Roaming\GIMP\ rmdir /s /q .\data\Users\MarioMasta64\AppData\Roaming\GIMP\
exit /b 2
...
set /p gimp_version=<.\doc\gimp_version.txt
taskkill /f /im "gimp-!gimp_version!.exe"
if exist .\bin\gimp\ rmdir /s /q .\bin\gimp\
if exist .\extra\gimp* del .\extra\gimp* >nul
exit /b 2
...
cls
title Portable Gimp Launcher - Helper Edition - Checking For Update
...
:a
:DLLDownloaderCheck
cls & title Portable Gimp Launcher - Helper Edition - Download Dll Downloader
...
:b
:PortableEverything
cls & title Portable Gimp Launcher - Helper Edition - Download Suite
...
title Portable Gimp Launcher - Helper Edition - Quicklauncher Writer
...
echo set "ProgramFiles=%%folder%%\data\Program Files">>!quick_launcher!
echo cls>>!quick_launcher!
echo set /p gimp_version=^<.\doc\gimp_version.txt>>!quick_launcher!
echo start "" ".\bin\gimp\bin\gimp-%%gimp_version%%.exe">>!quick_launcher!
...
call :HelperDownload "https://www.gimp.org/downloads/" "index.html"
set counter=0
:UpgradeSearchLoop
set /a counter+=1
for /f tokens^=%counter%delims^=^" %%A in (
  'findstr /i /c:".exe" index.html'
) Do > .\doc\gimp_link.txt Echo:%%A& goto ExitSearch
:ExitSearch
...
if exist index.html del index.html >nul
set "gimp_link=https:!gimp_link!"
set "gimp_link=!gimp_link:.torrent=!"
if "!Debug!" EQU "1" (
  cls
  echo "!gimp_link!"
  echo PRESS ENTER TO CONTINUE & pause >nul
)
set "gimp_temp=!gimp_link!"
set /a counter=0
:LoopSlashCheck
if "!gimp_temp:~-1!" NEQ "/" (
  set "gimp_temp=!gimp_temp:~0,-1!"
  set /a counter+=1
  goto LoopSlashCheck
)
if "%gimp_temp:~-1%"=="/" (
  set /a counter-=1
  echo !gimp_link:~-%counter%!>.\doc\gimp_exe.txt
)
...
cls
if exist ".\extra\!gimp_exe!" (
  echo gimp is updated.
  pause
  exit /b 2
)
cls
echo upgrading to gimp v!gimp_exe:~5,-13!
...
:e
title Portable Github Desktop Launcher - Helper Edition - Text-Reader Update Check
cls
...
if not exist ".\bin\gimp\bin\gimp*.exe" set nag=GIMP IS NOT INSTALLED CHOOSE "D"
exit /b 2
...
echo ================================================== >> !license!
cls
title Portable Gimp Launcher - Helper Edition - About
...
:LatestBuild
cls
title Portable Gimp Launcher - Helper Edition - Latest Build :D
...
:NewUpdate
cls
title Portable Gimp Launcher - Helper Edition - Old Build D:
if "!NoPrompt!" NEQ "1" (
  cls
  echo !NAG!
  set nag="Selection Time!"
...
:UpdateNow
cls & title Portable Gimp Launcher - Helper Edition - Updating Launcher
...
:PreviewBuild
cls
title Portable Gimp Launcher - Helper Edition - Test Build :0
...
:Cmd
cls
title Portable Gimp Launcher - Helper Edition - Command Prompt - By MarioMasta64
ver
